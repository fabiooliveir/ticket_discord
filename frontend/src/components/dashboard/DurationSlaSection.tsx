...
import { AccessTime, Timer, Speed, Assessment } from '@mui/icons-material';
...
const DurationSlaSection: React.FC<DurationSlaSectionProps> = ({
  data,
  loading = false,
}) => {
  ...
  return (
    <Paper sx={{ p: 3, mb: 3 }}>
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <MetricCard
            ...
            color={
              durationSla.durationComplianceRate >= 90
                ? 'success'
                : durationSla.durationComplianceRate >= 70
                  ? 'warning'
                  : 'error'
            }
            icon={<Assessment />}
      ...
      <Grid container spacing={2}>
        {Object.entries(durationSla.performanceByPriority).map(
          ([priority, metrics]) => {
      ...
      {Object.values(durationSla.performanceByPriority).every(
